"""
process_pines_data.py

Reads today's scraped data (data/pines_raw.xlsx) and adds it to the
starting values (data/starting_values.json) to produce an updated
output (data/pines_processed.json).
...
Flow:
  ...
  5. Save updated pines_processed.json
...
def main():
    data_dir = Path("data")
    raw_file = data_dir / "pines_raw.xlsx"
    starting_file = data_dir / "starting_values.json"
    output_file = data_dir / "pines_processed.json"
    
    print("🔄 Processing MP The Pines data...")
